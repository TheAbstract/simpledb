...
class Database:
    ...
    def __len__(self):
        self._sanity_check()
        with self._lock:
            item = self._sqlite.execute('SELECT COUNT(ID) FROM DATA').fetchone()
    ...
    def commit(self):
        ...
        with self._lock:
            self._sqlite.commit()
            log(
                f'SFDB[{self._filename}] Committed {self._commit_counter} transactions during the \
                last {"%.2f" % (time.time() - self._commit_timer)} seconds.'
            )
            self._commit_timer = time.time()
            self._commit_counter = 0
    ...
    def keys(self):
        self._sanity_check()
        with self._lock:
            return [item[0] for item in self._sqlite.execute('SELECT ID FROM DATA').fetchall()]

    def todict(self):
        self._sanity_check()
        with self._lock:
            return {
                item[0]: json.loads(item[1]) for item in self._sqlite.execute('SELECT ID, JSON FROM DATA').fetchall()
            }
